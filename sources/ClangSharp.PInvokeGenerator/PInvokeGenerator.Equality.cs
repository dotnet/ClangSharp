...
public partial class PInvokeGenerator
{
    ...
    // translation unit rather than per record. The USR set holds every record that a named `--with-equality-members`
    ...
    // Decides whether a record should get field-wise equality. `--with-equality-members` names the opted-in structs
    ...
    {
        if (_config._withEqualityMembers.Count == 0)
        {
            return false;
        }
    ...
    private void EnsureEqualityClosure(RecordDecl recordDecl)
    {
        ...
        foreach (var candidate in EnumerateRecordDecls(translationUnit.TranslationUnitDecl))
        {
            ...
            if (HasRemapping(candidate, _config._withEqualityMembers, matchStar: false)
    ...
    // null when the record is not a safe candidate for `--with-equality-members`. Element-wise equality is only

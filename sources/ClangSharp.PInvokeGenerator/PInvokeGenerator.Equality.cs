...
public partial class PInvokeGenerator
{
    ...
    // Tracks the translation unit the equality closure below was built for, so it is recomputed once per
    // translation unit rather than per record. The USR set holds every record that a named `--with-equality`
    // entry pulls in transitively (see EnsureEqualityClosure).
    private TranslationUnit? _equalityClosureTranslationUnit;
    private readonly HashSet<string> _equalityClosureUsrs = new(StringComparer.Ordinal);

    // Decides whether a record should get field-wise equality. `--with-equality` names the opted-in structs
    // (with `*` meaning every eligible struct); nothing is generated by default because element-wise equality
    // isn't universally valid (padding, exported comparison functions). A named struct also implicitly opts
    // in the nested/base structs it compares by value so its Equals(TSelf) binds to their generated Equals
    // rather than silently boxing through ValueType.Equals.
    private bool ShouldGenerateEqualityFor(RecordDecl recordDecl)
    {
        if (_config._withEqualities.Count == 0)
        {
            return false;
        }

        if (HasRemapping(recordDecl, _config._withEqualities, matchStar: true))
        {
            return true;
        }

        EnsureEqualityClosure(recordDecl);

        var usr = recordDecl.Handle.Usr.CString;
        return !string.IsNullOrEmpty(usr) && _equalityClosureUsrs.Contains(usr);
    }

    // Builds the transitive by-value dependency closure of every explicitly-named record in the translation
    // unit. Those dependencies are usually declared (and emitted) before the struct that names them, so the
    // closure is precomputed for the whole translation unit instead of relying on visitation order.
    private void EnsureEqualityClosure(RecordDecl recordDecl)
    {
        var translationUnit = recordDecl.TranslationUnit;

        if (ReferenceEquals(translationUnit, _equalityClosureTranslationUnit))
        {
            return;
        }

        _equalityClosureTranslationUnit = translationUnit;
        _equalityClosureUsrs.Clear();

        foreach (var candidate in EnumerateRecordDecls(translationUnit.TranslationUnitDecl))
        {
            // `*` is already handled by the direct match in ShouldGenerateEqualityFor, so only explicitly
            // named records seed the closure, and only when they are themselves field-wise comparable.
            if (HasRemapping(candidate, _config._withEqualities, matchStar: false)
                && IsRecordEqualityEligible(candidate, candidate as CXXRecordDecl, fields: null, []))
            {
                CollectEqualityClosure(candidate, _equalityClosureUsrs);
            }
        }
    }

    private void CollectEqualityClosure(RecordDecl recordDecl, HashSet<string> closure)
    {
        var usr = recordDecl.Handle.Usr.CString;

        if (!string.IsNullOrEmpty(usr) && !closure.Add(usr))
        {
            return;
        }
        ...
        if (recordDecl is CXXRecordDecl cxxRecordDecl)
        {
            ...
            {
                CollectEqualityClosure(GetRecordDecl(cxxBaseSpecifier), closure);
            }
        }

        foreach (var fieldDecl in recordDecl.Fields)
        {
            if (fieldDecl.Type.CanonicalType is RecordType recordType)
            {
                CollectEqualityClosure(recordType.Decl, closure);
            }
        }
    }

    private static IEnumerable<RecordDecl> EnumerateRecordDecls(Cursor cursor)
    {
        foreach (var child in cursor.CursorChildren)
        {
            if (child is RecordDecl recordDecl)
            {
                yield return recordDecl;
            }

            foreach (var nested in EnumerateRecordDecls(child))
            {
                yield return nested;
            }
        }
    ...
    // null when the record is not a safe candidate for `--with-equality`. Element-wise equality is only
    // ever opt-in because it is not universally valid (padding, exported comparison functions), so anything
    // that cannot be compared field-for-field here is left untouched rather than guessed at.
    private List<EqualityField>? GetEqualityFields(RecordDecl recordDecl, CXXRecordDecl? cxxRecordDecl)
    {
        if (!ShouldGenerateEqualityFor(recordDecl))
        {
            return null;
        }
